...
module.exports.run = async (client, message, args) => {
  ...
  if (message.member.permissions.has(8) || message.member.roles.cache.some(e => arr.some(x => x == e))) {
      ...
      if (!user) return message.reply(`**Lütfen bir kişi etiketleyin veya ID yazınız** \`(örn: ${prefix[0]}erkek @Thelox Thelox 18)\``).then(x => x.delete({timeout: 5000}));
        ...
        if (user.user.username.includes(sunucutagi)) {
          if (client.channels.cache.get(await db2.get(`sunucuayar.sohbet_kanal`))) {
            client.channels.cache.get(await db2.get(`sunucuayar.sohbet_kanal`)).send(`${client.emojis.cache.find(x => x.name === "thelox_sag")} ${user} (\`${user.id}\`) adlı üye başarılı bir şekilde kayıt oldu! Şuan da **${message.guild.memberCount}** kişiyiz!`).then(x => x.delete({timeout: 5000}))
          }
          ...
          if (client.channels.cache.get(db2.get(`sunucuayar.sohbet_kanal`))) {
            client.channels.cache.get(db2.get(`sunucuayar.sohbet_kanal`)).send(`${client.emojis.cache.find(x => x.name === "thelox_sag")} ${user} (\`${user.id}\`) adlı üye başarılı bir şekilde kayıt oldu! Şuan da **${message.guild.memberCount}** kişiyiz!`).then(x => x.delete({timeout: 5000}))
          }
