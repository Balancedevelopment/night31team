...
exports.run = async (client, message, args) => {
  ...
  if (gkv.some(i => i == message.author.id) || message.author.id === message.guild.ownerID) {
    const sec = args[0]
    let emoji = message.guild.emojis.cache.find(x => x.name === "thelox_tik")
    let emoji2 = message.guild.emojis.cache.find(x => x.name === "thelox_iptal");
    let emoji3 = message.guild.emojis.cache.find(x => x.name === "thelox_onay")
...
${emoji3} **Başarılı bir şekilde ${rol} rolünü yaptınız **`)).then(xyz => xyz.delete({
    ...
    if (sec == "booster-rol" || sec == "booster") {
      const rol = message.mentions.roles.first()
      if (!rol) {
