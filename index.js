...
client.ayarlar = {
  "prefix": [".", "!", "-", "?"],
  "token": "ODU3NTQzNDQzMTM5MjY0NTEy.YNRHkQ.w2PGC8W-1aJpLYAdEqrtNCydAxQ",
  "yapimci": ["767836565937586219"],
  // 515 ve 521 SATIDAKİ "TAG" yazan yerlere tagınızı koyun. Thelox Tag rol sistemi
  "tag": "TAG",
  "sunucu": "SUNUCU ID",
  "tagkanal":"TAG KANAL ID",
  "tagrol": "TAG ROL ID" ,
  // Thelox Tag rol sistemi
  "footer": "Thelox 💛",
  "readyFooter": ["Thelox 💛"],
...
client.on("message", async (msg) => {
  ...
  if (reklamKoruma) {
    try {
      const kelime = ["discord.gg", "discord.me", "discordapp.com", "discord.io", "discord.tk",".gg"];
    ...
    try {
      ...
      if (kufurler.some(word => args.some(c => word.toLowerCase() == c.toLowerCase()))) {
        if (!msg.member.permissions.has(8)) {
          msg.delete().then(msg => {
            msg.reply(" it's haram bro.").then(m => m.delete({
              timeout: 2000
            })).catch(err => {});
          });
        }
    ...
    try {
      ...
      if (kufurler.some(word => args.some(c => word.toLowerCase() == c.toLowerCase()))) {
        newMsg.delete().then(message => {
          newMsg.reply(" it's haram bro.").then(m => m.delete({
            timeout: 2000
          })).catch(err => {});
        });
      }
...
client.on("userUpdate", async (oldUser, newUser) => { //acebots 
  ...
  //acebots
          let tag = client.ayarlar.tag
          let sunucu = client.ayarlar.sunucu
          let kanal = client.ayarlar.tagkanal//acebots //acebots 
          let rol = client.ayarlar.tagrol
...
// say komutundaki emojiler
const sayiEmojiler = {
  0: `<a:thelox_sifir:885552175512686612>`, 
  1: `<a:thelox_bir:885552167925198958>`, 
  2: `<a:thelox_iki:885552153383534662>`, 
  3: `<a:thelox_uc:885552251534446612>`, 
  4: `<a:thelox_dort:885552220148478002>`, 
  5: `<a:thelox_bes:885552257167401020>`, 
  6: `<a:thelox_alti:885552185717448775>`, 
  7: `<a:thelox_yedi:885552162225148024>`, 
  8: `<a:thelox_sekiz:885552204084297739>`, 
  9: `<a:thelox_dokuz:885552229493395506>` 
};
...
function cezalandir(kisiID, tur) {
  client.guilds.cache.map(x => {
    ...
    if (tur == "ban") return uye.ban({ reason: "Thelox Koruma" }).catch();
  });
};
